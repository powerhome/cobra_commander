...
RSpec.describe "cli", type: :aruba do
  ...
  describe "counting dependencies on a component in the tree" do
    it "counts a component's direct dependency" do
      run_simple("cobra dependents_of #{@root} --component=node_manifest --format=list", fail_on_error: true)
    ...
    it "counts a component's transient dependency" do
      run_simple("cobra dependents_of #{@root} --component=g --format=count", fail_on_error: true)

      expect(last_command_started.output.to_i).to eq(3)
    end

    it "doesn't count a component it's not dependent on" do
      run_simple("cobra dependents_of #{@root} --component=non_existent --format=list", fail_on_error: true)
